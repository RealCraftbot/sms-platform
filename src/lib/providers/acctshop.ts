...
export class AcctshopProvider extends BaseProvider {
  readonly name = "AcctShop"
  readonly slug = "acctshop"
  readonly category: "SMS" | "BOOSTING" | "ACCOUNTS" = "ACCOUNTS"
  readonly baseUrl = "https://acctshop.com/api"
  ...
  async getCost(service: string, country?: string): Promise<CostResult> {
    try {
      const response = await fetch(`${this.baseUrl}/price/${service}`, {
        headers: this.authHeaders(),
      })
      const data = await response.json()
